...
export class ErrorMiddleware {
    ...
    public static methodNotAllowed = (allowedMethods: Array<string>, req: Request, res: Response, next: NextFunction): void => {

        if (!allowedMethods.includes(req.method)) {
            next(AppError.methodNotAllowed(`Method ${req.method} not allowed on ${req.originalUrl}. Allowed methods: ${allowedMethods.join(', ')}`));
        } else {
            next();
        }
    };
